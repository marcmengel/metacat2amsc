...
class AmSCClient:
    ...
    def post_create(self, entity_dict):
        ...
        if resp.status_code != 200:
            ...
            if resp.text.find("already exists") > 0:
                ...
                logger.error(
                    f"got status {resp.status_code} for POST catalog entry: {resp.text}"
                )
    ...
    def put_update(self, entity_dict):
        ...
        if resp.status_code != 200:
            logger.error(
                f"Error: got status {resp.status_code} for PUT catalog entry: {resp.text}"
            )
    ...
    def delete_item(self, entity):
        ...
        if resp.status_code != 200:
            logger.error(
                f"Error: got status {resp.status_code} for DELETE catalog entry: {resp.text}"
            )
            # raise RuntimeError(f"got status {resp.status_code} for PUT catalog entry: {resp.text}")
        return resp.json()
